...
## Чек‑лист ревью (для ментора)

- [ ] **Главное:** студент понимает разделение **Entity ↔ DTO**. Прямой возврат `BookEntity` из контроллера — серьёзный антипаттерн (утечка JPA‑деталей в API)
- [ ] **Constructor injection** везде, не `@Autowired` на полях
- [ ] Правильное использование HTTP статусов: 201 Created на POST, 204 No Content на DELETE, 404 Not Found на отсутствующее
- [ ] `@Transactional` на сервисе (не на контроллере)
- [ ] DTO валидируется в сервисе через `require` — и это превращается в 400 через ControllerAdvice
- [ ] Тесты — **интеграционные** через `MockMvc`, не unit‑тесты на контроллер с моком сервиса (ну или и то, и то — но MockMvc обязателен)
- [ ] Понимает, зачем нужны `kotlin-spring` и `kotlin-jpa` плагины — спросите его
- [ ] **Отдельная архитектурная тема:** студент **не** делает контроллер «жирным» — там должно быть только маппинг HTTP‑деталей. Логика — в сервисе
